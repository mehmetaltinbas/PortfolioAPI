...
const GetAllByExperienceIdAsync = errorHandler(async function ActivityService_GetAllByExperienceIdAsync(experienceId) {
    const activities = await models.Activity.find({ experienceId }).sort({ order: 1 });
...
const UpdateAsync = errorHandler(async function ActivityService_UpdateAsync(data) {
    ...
    const activity = await models.Activity.findOne({ _id: activityId, experienceId });
    if (!activity) return { isSuccess: false, message: "Activity not found or unauthorized" };
...
const DeleteAsync = errorHandler(async function ActivityService_DeleteAsync(data) {
    ...
    const activity = await models.Activity.findOneAndDelete({ _id: activityId, experienceId });
    if (!activity) return { isSuccess: false, message: "Activity not found or unauthorized" };
