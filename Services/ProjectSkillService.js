...
const CreateAsync = errorHandler(async function ProjectSkillService_CreateAsync(data) {
    ...
    return {
        isSuccess: true,
        message: 'Project skill created successfully',
    };
});

const GetAllByProjectIdAsync = errorHandler(
    async function ProjectSkillService_GetAllByProjectIdAsync(projectId) {
        const projectSkills = await models.ProjectSkill.find({ projectId });
        return {
            isSuccess: true,
            message: 'Project skills associated with given projectId read.',
            projectSkills,
        };
    },
);
...
const UpdateAsync = errorHandler(async function ProjectSkillService_UpdateAsync(data) {
    ...
    if (!updatedProjectSkill)
        return {
            isSuccess: false,
            message: 'Project skill not found or unauthorized',
        };
