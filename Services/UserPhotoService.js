...
const CreateAsync = errorHandler(async function UserPhotoService_CreateAsync(data) {
    const currentUserPhoto = await models.UserPhoto.findOne({
        type: data.type,
    });
    if (!currentUserPhoto) {
...
const GetAllByUserIdAsync = errorHandler(
    async function UserPhotoService_GetAllByUserIdAsync(userId) {
        ...
        return {
            isSuccess: true,
            message: 'User photos associated with given userId read.',
            userPhotos,
        };
    },
);
...
const UpdateAsync = errorHandler(async function UserPhotoService_UpdateAsync(data) {
    ...
    if (!updatedUserPhoto)
        return {
            isSuccess: false,
            message: 'User photo not found or unauthorized',
        };
...
const DeleteAsync = errorHandler(async function UserPhotoService_DeleteAsync(data) {
    ...
    const deletedUserPhoto = await models.UserPhoto.findOneAndDelete({
        _id: userPhotoId,
    });
    if (!deletedUserPhoto)
        return {
