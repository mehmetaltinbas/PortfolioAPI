...
const GetAllByUserIdAsync = errorHandler(
    async function SkillService_GetAllByUserIdAsync(userId) {
        const skills = await models.Skill.find({ userId });
        return {
            isSuccess: true,
            message: 'Skills associated with given userId read.',
            skills,
        };
    },
);
...
const DeleteAsync = errorHandler(async function SkillService_DeleteAsync(data) {
    ...
    const deletedSkill = await models.Skill.findOneAndDelete({
        _id: skillId,
        userId,
    });
