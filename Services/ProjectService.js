    ...
    async function ProjectService_GetAllByUserIdAsync(userId) {
        ...
        await Promise.all(
            projects.map(async (project) => {
                ...
                const projectSkillsResponse = await projectSkillService.GetAllByProjectIdAsync(
                    project._id,
                );
                project.projectSkills = projectSkillsResponse.projectSkills;
                const projectPhotosResponse = await projectPhotoService.GetAllByProjectIdAsync(
                    project._id,
                );
...
const DeleteAsync = errorHandler(async function ProjectService_DeleteAsync(data) {
    ...
    const deletedProject = await models.Project.findOneAndDelete({
        _id: projectId,
        userId,
    });
