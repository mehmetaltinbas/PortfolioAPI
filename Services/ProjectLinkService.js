...
const GetAllByProjectIdAsync = errorHandler(
    async function ProjectLinkService_GetAllByProjectIdAsync(projectId) {
        const projectLinks = await models.ProjectLink.find({ projectId });
        return {
            isSuccess: true,
            message: 'Project links associated with given projectId read.',
            projectLinks,
        };
    },
);
...
const UpdateAsync = errorHandler(async function ProjectLinkService_UpdateAsync(data) {
    ...
    if (!updatedProjectLink)
        return {
            isSuccess: false,
            message: 'Project link not found or unauthorized',
        };
...
const DeleteAsync = errorHandler(async function ProjectLinkService_DeleteAsync(data) {
    ...
    const deletedProjectLink = await models.ProjectLink.findOneAndDelete({
        _id: projectLinkId,
        projectId,
    });
